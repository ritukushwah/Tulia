...
class frontCommon_Controller extends MX_Controller {
   ...
   public function image_unlink($filename, $filepath) {
    ...
    function check_user_session($page_slug =''){

        $allowed_pages = array('index','login'); //these pages do not require user authentication
        if(empty($this->session->userdata['id']) && (in_array($page_slug,$allowed_pages))){
            return TRUE;
        }else{
            ...
            //user session exists
            $where = array('id'=>$this->session->userdata['id'],'status'=>1);
            ...
            if(in_array($page_slug,$after_auth)){
                redirect('');
            }else{
                return TRUE; 
            }
    ...
    function check_ajax_auth(){
        ...
        $failed_res = json_encode(array('status'=>-1,'message'=>'Your session expired. Please login again.','url'=>base_url().'login'));
        if(empty($this->session->userdata['id'])){ 
            return $failed_res;
        }
